...
| 6 | **Beta installer + distributable package** | not started | [v1-plan-6-beta-installer.md](v1-plan-6-beta-installer.md) | One-click Inno Setup `.exe` (+ fallback `.zip`) that assembles a clean plugin folder and drops it in the detected C4D 2026 plugins location. Unsigned for beta (one-time SmartScreen click-through, documented in the manual); code-signing deferred to public release. New `scripts/package.ps1` produces the version-stamped distributable from a Release build. Sequenced after Plan 5. |

Eight plans. v1 ships when all eight land verified.
